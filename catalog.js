...
for(const slot of document.querySelectorAll('.picture-slot')){if(slot.querySelector('img'))continue;const path=window.IDLE_HUNTER_IMAGES?.[slot.dataset.kind]?.[slot.dataset.name];if(!path)continue;const img=document.createElement('img');img.alt=slot.dataset.name;img.loading='lazy';img.width=320;img.height=180;img.addEventListener('load',()=>{slot.removeAttribute('aria-hidden');slot.classList.add('has-custom-image');});img.addEventListener('error',()=>img.remove());img.src=path;slot.append(img);}
